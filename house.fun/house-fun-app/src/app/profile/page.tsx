...
function ProfileContent() {
  ...
  const { data: stats, isLoading: isLoadingStats } = usePlayerStats();
  const { data: bets, isLoading: isLoadingBets } = usePlayerBets(20, 0);
                ...
                <div key={bet.id} className="flex items-center justify-between p-4 bg-white/5 rounded-xl hover:bg-white/10 transition-colors">
                  <div className="flex items-center gap-4">
                    <div className={`size-10 rounded-lg flex items-center justify-center ${
                      bet.playerWon ? 'bg-primary/20 text-primary' : 'bg-danger/20 text-danger'
                    }`}>
