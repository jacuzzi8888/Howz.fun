...
const FlipItGameContent: React.FC = () => {
    ...
    const handleFlip = async () => {
        ...
        try {
            ...
            if (isDemoMode) {
                console.log('[FlipIt] Running in Presentation Mode (Demo)');

                // Mock sequence for perfect video recording
                try {
                    setTxStatus('pending');
                    ...
                    const mockBet = {
                        ...
                        betPDA: new PublicKey("Howz111111111111111111111111111111111111111"),
                        commitment: new Uint8Array(32),
                    };

                    setBetResult(mockBet as any);
                    ...
                    setGameResult(mockResolvedResult as any);
                    setShowResult(true);
                    setTxStatus('confirmed');
                    play(win ? 'win' : 'loss');
                    return;
                } catch (err) {
                    console.error('[FlipIt] Demo error:', err);
                    setTxStatus('failed');
                    return;
                }
            }
        ...
        <div className="flex flex-1 relative overflow-y-auto overflow-x-hidden">
            ...
            <div className="flex-1 flex flex-col items-center justify-start pt-6 py-4 px-3 sm:px-4 lg:px-6 relative z-10 min-w-0 min-h-[500px]">
                <div className="w-full max-w-lg flex flex-col items-center gap-4 bg-black/5 rounded-3xl p-2 sm:p-4">
                    ...
                    {/* Game Result Modal */}
                    <GameResultModal
                        isOpen={showResult && !!gameResult}
                        playerWon={gameResult?.playerWon ?? false}
                        title={gameResult?.playerWon ? 'You Won!' : 'House Wins'}
                        subtitle={gameResult ? `Outcome: ${gameResult.outcome}` : undefined}
                        amount={gameResult?.playerWon ? gameResult.payout : undefined}
                        betAmount={!gameResult?.playerWon ? amount : undefined}
                        gameName="Flip It"
